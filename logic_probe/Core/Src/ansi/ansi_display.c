...
void ansi_print_logo(const char* color, const char* bg_color) {
    ...
    for (unsigned i = 0; i < art_rows; ++i) {
        ...
        ansi_send_text(jlp_art[i], color, bg_color);
    }

    char* made_by = "Made by Milan Jiříček";
    ansi_set_cursor(row + art_rows - 1, TERMINAL_WIDTH / 2 - strlen(made_by) / 2);
    ansi_send_text(made_by, WHITE_TEXT, "");
...
void ansi_print_border(const char* color, const char* bg_color) {
    ...
    memset(buffer, BORDER_HORIZONTAL, sizeof(buffer));
    buffer[TERMINAL_WIDTH - 1] = '\0';

    ansi_set_cursor(0, 0);
    ansi_send_text(buffer, color, bg_color);
    ansi_set_cursor(TERMINAL_HEIGHT - 1, 0);
    ansi_send_text(buffer, color, bg_color);

    for (unsigned int i = 2; i < TERMINAL_HEIGHT - 1; ++i) {
        ansi_set_cursor(i, 0);
        ansi_send_text(BORDER_VERTICAL, color, bg_color);
